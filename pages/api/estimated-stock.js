...
import EstimatedStock from '../../models/EstimatedStock';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const items = await EstimatedStock.find(filter).sort({ date: -1 }).limit(200);
    return res.json({ items });
  }

  if (req.method === 'POST') {
    if (![Roles.Admin, Roles.StockManager].includes(user.role)) return res.status(403).json({ error: 'Forbidden' });
    const item = await EstimatedStock.create(req.body);
  ...
  if (req.method === 'PUT') {
    ...
    const item = await EstimatedStock.findByIdAndUpdate(id, update, { new: true });
  ...
  if (req.method === 'DELETE') {
    if (![Roles.Admin, Roles.StockManager].includes(user.role)) return res.status(403).json({ error: 'Forbidden' });
    const { id } = req.query;
    await EstimatedStock.findByIdAndDelete(id);
